...
const Specialties: FC<Props> = ({ language, specialties }: Props) => {
  const theme = useTheme();
  const router = useRouter();
  const currentUser = useContext(UserContext);
  const [univers, setUnivers] = useState(specialties);

  const handleSearch = (event) => {
    setUnivers(specialties.filter(n => n.name.indexOf(event.currentTarget.value) !== -1));
  }
          ...
          >
            ...
            <Card disableBorder css={{ flex: 1, fontSize: 18, marginTop: 10, padding: 30, boxShadow: theme.blockShadowWide, background: "#fff", }}>
              ...
              <Button
                ...
                onClick={() => router.push(`/specialty/${n.id}`)}
              >More</Button>
            </Card>
          </Card>
        ))}
...
export const getServerSideProps: GetServerSideProps<Props> = async ctx => {
  ...
  const specialties = await (await controller.get(`/api/specialists`)).data.result;
  return { props: { language: locales[language], specialties } }
}
